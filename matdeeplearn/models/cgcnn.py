...
class CGCNN(torch.nn.Module):
    def __init__(
        ...
        :param dim2:
        :param pre_fc_count:
        :param gc_count: the count of graph convolutional layers
        :param post_fc_count:
        :param pool:
        :param pool_order:
        :param batch_norm:
        :param batch_track_stats:
        :param act:
        :param dropout_rate:
        """
        ...
        # region define layers with torch.nn.ModuleList
        # Set up pre-GNN dense layers (NOTE: in v0.1 this is always set to 1 layer)
        if pre_fc_count > 0:
            self.pre_lin_list = torch.nn.ModuleList()  # will be visible by all Module methods.
        ...
        for i in range(gc_count):
            # https://pytorch-geometric.readthedocs.io/en/latest/_modules/torch_geometric/nn/conv/cg_conv.html
            conv = CGConv(gc_dim, data.num_edge_features, aggr="mean", batch_norm=False)
        ...
        # Should processing_set ups be a hyper-parameter?
        if self.pool_order == "early" and self.pool == "set2set":
    ...
    def forward(self, data):
        ...
        for i in range(0, len(self.pre_lin_list)):
            if i == 0:
                ...
                out = getattr(F, self.act)(out)  # active function
            else:
                out = self.pre_lin_list[i](out)
                out = getattr(F, self.act)(out)  # active function

        # GNN layers
        for i in range(0, len(self.conv_list)):  # conv_list is list of GNN layers
            if len(self.pre_lin_list) == 0 and i == 0:  # if no Pre-GNN linear dense layers
                if self.batch_norm == "True":
